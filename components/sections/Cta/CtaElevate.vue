<script setup lang="ts">
interface Benefits {
  name: string
}



const query = groq`*[ _type == "benefit"]{
  name
}`
const sanity = useSanity()

const { data: benefits } = await useAsyncData('benefits', () =>
  sanity.fetch<{ benefits: Benefits }>(query)
)
</script>

<template>
  <section class="py-24 bg-gray-900 sm:py-32">
    <div class="relative isolate">
      <UContainer>
        <div
          class="mx-auto flex max-w-2xl flex-col gap-16 bg-white/5 px-6 py-16 ring-1 ring-white/10 sm:rounded-3xl sm:p-8 lg:mx-0 lg:max-w-none lg:flex-row lg:items-center lg:py-20 xl:gap-x-20 xl:px-20"
        >
          <NuxtImg
            class="h-96 w-full flex-none rounded-2xl object-cover shadow-xl lg:aspect-square lg:h-auto lg:max-w-sm"
            src="https://images.unsplash.com/photo-1530443938569-d2765de35c16?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2940&q=80"
            alt="Elevate Your Business"
          />
          <div class="w-full flex-auto">
            <h2
              class="text-3xl font-bold tracking-tight text-white sm:text-4xl"
            >
              Elevate Your Business
            </h2>
            <p class="mt-6 text-lg leading-8 text-gray-300">
              Join Elite Business Connections and be part of a community that
              fosters growth, innovation, and success.
            </p>
            <ul
              class="mt-10 grid grid-cols-1 gap-x-8 gap-y-3 text-base leading-7 text-white sm:grid-cols-2"
              role="list"
            >
              <li
                class="flex gap-x-3"
                v-for="benefit in benefits"
                :key="benefit.name"
              >
                <Icon
                  class="h-7 w-5 flex-none text-primary"
                  name="heroicons:check-circle-solid"
                  aria-hidden="true"
                />
                {{ benefit.name }}
              </li>
            </ul>
            <div class="mt-10 flex">
              <NuxtLink
                class="text-sm font-semibold leading-6 text-primary"
                to="/contact"
                >Learn More About EBC
                <span aria-hidden="true">&rarr;</span></NuxtLink
              >
            </div>
          </div>
        </div>
      </UContainer>
      <div
        class="absolute inset-x-0 -top-16 -z-10 flex transform-gpu justify-center overflow-hidden blur-3xl"
        aria-hidden="true"
      >
        <div
          class="aspect-[1318/752] w-[82.375rem] flex-none bg-gradient-to-r from-[#80caff] to-[#4f46e5] opacity-25"
          style="
            clip-path: polygon(
              73.6% 51.7%,
              91.7% 11.8%,
              100% 46.4%,
              97.4% 82.2%,
              92.5% 84.9%,
              75.7% 64%,
              55.3% 47.5%,
              46.5% 49.4%,
              45% 62.9%,
              50.3% 87.2%,
              21.3% 64.1%,
              0.1% 100%,
              5.4% 51.1%,
              21.4% 63.9%,
              58.9% 0.2%,
              73.6% 51.7%
            );
          "
        />
      </div>
    </div>
  </section>
</template>
