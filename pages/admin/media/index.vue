<template>
  <div class="p-6">
    ...
    <UCard>
      ...
      <!-- Grid View -->
      <div v-if="viewMode === 'grid'" class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        <UCard
      ...
      <UCard>
        <template #header>
          <div class="flex items-center">
            <UIcon name="i-heroicons-arrow-up-tray" class="mr-2 text-primary-500" />
        ...
        <div>
          ...
          >
            ...
            <p class="mt-1 text-sm text-gray-500">
              Upload multiple files at once. Images, documents, and other files supported.
            </p>
          </div>
      ...
      <UCard class="max-w-xl">
        ...
        <div v-if="selectedFile" class="space-y-4">
          <div class="flex justify-center">
            <div v-if="isImage(selectedFile)" class="h-48 overflow-hidden rounded">
              <img
...
const getFileIcon = (file: Media) => {
  ...
  if (file.mimetype.includes('pdf')) return 'i-heroicons-document-text'
  if (file.mimetype.includes('word')) return 'i-heroicons-document-text'
  if (file.mimetype.includes('excel') || file.mimetype.includes('spreadsheet')) return 'i-heroicons-table-cells'
  if (file.mimetype.includes('presentation')) return 'i-heroicons-presentation-chart-bar'
...
const getFileIconByType = (mimeType: string) => {
  ...
  if (mimeType.includes('excel') || mimeType.includes('spreadsheet')) return 'i-heroicons-table-cells'
  if (mimeType.includes('presentation')) return 'i-heroicons-presentation-chart-bar'
...
const deleteFile = async () => {
  ...
  try {
    ...
    mediaFiles.value = mediaFiles.value.filter(file => file.id !== fileToDelete.value?.id)
    totalFiles.value--
