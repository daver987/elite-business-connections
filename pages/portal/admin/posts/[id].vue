...
onMounted(async () => {
  ...
  if (post) {
    ...
    state.published = post.published
    state.featuredImage = post.featuredImage || ''
    
    // Update editor content if it's initialized
    if (editor.value) {
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      toast.add({
        title: 'Error',
        description: response.error || 'Failed to update post. Please try again.',
...
const deletePost = async () => {
  try {
    ...
    if (response.statusCode === 200) {
      ...
      console.error('Error deleting post:', response.error || 'Unknown error')
      toast.add({
        title: 'Error',
        description: response.error || 'Failed to delete post. Please try again.',
        icon: 'i-heroicons-exclamation-circle',
        color: 'red',
        timeout: 5000,
      })
...
<template>
  <UDashboardPage>
    <UDashboardPanel grow>
      <UDashboardNavbar :title="loading ? 'Loading...' : notFound ? 'Post Not Found' : `Edit Post: ${state.title}`">
        <template #right>
      ...
      <UDashboardPanelContent>
        <div v-if="loading" class="flex justify-center items-center h-64">
          <UIcon class="animate-spin h-8 w-8 text-gray-400" name="i-heroicons-arrow-path" />
          <span class="ml-2">Loading post...</span>
        </div>
        
        <div v-else-if="notFound" class="flex flex-col justify-center items-center h-64">
          <UIcon class="h-16 w-16 text-gray-400 mb-4" name="i-heroicons-exclamation-triangle" />
          <h2 class="text-xl font-medium text-gray-700 mb-2">Post Not Found</h2>
          <p class="text-gray-500 mb-4">The post you're looking for doesn't exist or has been deleted.</p>
          <UButton to="/portal/admin/posts" color="primary">
            Back to Posts
          </UButton>
        </div>
